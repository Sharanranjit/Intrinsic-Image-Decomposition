...
class BasicPolicy(object):
    ...
    def __call__(self, img, albedo, shading):
    ...
    def debug_img(self, img, output, idx, i, prefix=''):
        from PIL import Image
        albedo = output[0]
        shading = output[1]
        aug_img = Image.fromarray(np.clip(np.uint8(img*255), 0, 255))
        aug_img.save(prefix+str(idx)+"_"+str(i)+'.jpg',quality=99)
        aug_img = Image.fromarray(np.clip(np.uint8(albedo*255), 0, 255))
        aug_img.save(prefix+str(idx)+"_"+str(i)+'.jpg',quality=99)
        aug_img = Image.fromarray(np.clip(np.uint8(shading*255), 0, 255))
        aug_img.save(prefix+str(idx)+"_"+str(i)+'.jpg',quality=99)
